...
-- Packing details required (Yes/No) — SO-level, in Purchase Order Details.
-- Replaces the old per-EC "documents required" fields on Planning + Assembly &
-- Packing (those columns stay in the schema harmlessly; the app just no longer
-- exposes them).
...
CREATE TABLE IF NOT EXISTS order_packing_slips (
    id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    item_id           UUID NOT NULL REFERENCES order_items(id) ON DELETE CASCADE,
    seq               BIGINT GENERATED BY DEFAULT AS IDENTITY,
    -- 'tentative' = entered by Planning, 'actual' = entered by Packing.
    kind              TEXT NOT NULL DEFAULT 'actual'
                      CHECK (kind IN ('tentative', 'actual')),
    packing_slip_no   TEXT,
    packing_slip_date DATE,
    -- Export-client extras.
    box_size          TEXT,
    marking_on_case   TEXT,
    description       TEXT,   -- description of PUMP or SPARE
    quantity          INTEGER,
    item_weight       NUMERIC(14,3),
    gross_weight      NUMERIC(14,3),
    net_weight        NUMERIC(14,3),
    created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS order_packing_slips_item_id_idx
    ON order_packing_slips (item_id);
DROP TRIGGER IF EXISTS order_packing_slips_set_updated_at ON order_packing_slips;
CREATE TRIGGER order_packing_slips_set_updated_at BEFORE UPDATE ON order_packing_slips
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
...
CREATE TABLE IF NOT EXISTS order_invoices (
    ...
    vehicle_size      TEXT,
    vehicle_weight    TEXT,
    courier_mode      TEXT,   -- Normal / By Air
    freight_value     NUMERIC(14,2),
    delivery_date     DATE,
    -- Phase 3 — docket / LR details.
    docket_type       TEXT,   -- Export / Direct / Hundi / COD
    docket_no         TEXT,
    booking_date      DATE,
    material_weight   NUMERIC(14,3),
    per_kg_rate       NUMERIC(14,2),
    so_freight_terms  TEXT,
    delivery_charges  NUMERIC(14,2),
...
DROP TRIGGER IF EXISTS order_invoices_set_updated_at ON order_invoices;
CREATE TRIGGER order_invoices_set_updated_at BEFORE UPDATE ON order_invoices
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- Dispatch status is derived from invoiced quantity/value vs the SO's, so it
-- moves to the SO and is no longer hand-entered by Assembly & Packing.
ALTER TABLE orders ADD COLUMN IF NOT EXISTS dispatch_status TEXT;
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_name='order_assembly_dispatch' AND column_name='dispatch_status') THEN
        UPDATE orders o SET dispatch_status = COALESCE(o.dispatch_status, sub.s)
          FROM (
            SELECT it.order_id, MAX(ad.dispatch_status) AS s
              FROM order_assembly_dispatch ad
              JOIN order_items it ON it.id = ad.item_id
             GROUP BY it.order_id
          ) sub WHERE sub.order_id = o.id;
        ALTER TABLE order_assembly_dispatch DROP COLUMN dispatch_status;
    END IF;
END $$;

-- Target Date for Dispatch Team moves to the SO (Order details → Target Dates),
-- alongside the other targets Central Visibility sets at intake. Idempotent.
ALTER TABLE orders ADD COLUMN IF NOT EXISTS dispatch_team_target_date DATE;
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_name='order_assembly_dispatch'
                  AND column_name='dispatch_team_target_date') THEN
        UPDATE orders o SET
            dispatch_team_target_date = COALESCE(o.dispatch_team_target_date, sub.d)
          FROM (
            SELECT it.order_id, MAX(ad.dispatch_team_target_date) AS d
              FROM order_assembly_dispatch ad
              JOIN order_items it ON it.id = ad.item_id
             GROUP BY it.order_id
          ) sub WHERE sub.order_id = o.id;
...
-- Client zone, seeded from Market Intell when an order is created from the
-- client directory (clients.zone → tour_routes.zone → rep's users.zone).
ALTER TABLE orders ADD COLUMN IF NOT EXISTS zone TEXT;

-- Client zone, seeded from Market Intell when an order is created from the
-- client directory. Sourced from tour_routes.zone (the client's territory via
-- clients.tour_id) — clients.zone itself is unpopulated upstream, and the
-- rep's users.zone is the rep's own zone, not the client's.
ALTER TABLE orders ADD COLUMN IF NOT EXISTS zone TEXT;

-- Spare counterparts to the pump-only Planning fields. Each pair is gated on
...
-- one a Yes/No + date:
--   • issued to Client      (Drawing owns)
--   • approved              (Central Visibility owns — centralOnly in the schema)
--   • issued to Production  (Drawing owns)
-- The legacy order_drawing columns are left in place so historical values
-- aren't lost; they're simply no longer surfaced.
CREATE TABLE IF NOT EXISTS order_drawing_revisions (
    id                          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    item_id                     UUID NOT NULL REFERENCES order_items(id) ON DELETE CASCADE,
    seq                         BIGINT GENERATED BY DEFAULT AS IDENTITY,
    revision_no                 TEXT,
    issued_to_client            TEXT,   -- Yes / No
    issued_to_client_date       DATE,
    approved                    TEXT,   -- Yes / No  (Central Visibility)
    approved_date               DATE,
    issued_to_production        TEXT,   -- Yes / No
    issued_to_production_date   DATE,
    created_at                  TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at                  TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX IF NOT EXISTS order_drawing_revisions_item_id_idx
    ON order_drawing_revisions (item_id);
...
-- order_payment_terms
-- ===========================================================================
-- How an order is to be paid, one row per slice of it: "30% Advance Against
-- ABG", "40% Before Dispatch", "30% After Receipt, 45 days". Terms are added
-- to, not revised: the order's own text column keeps whatever prose the PO
-- used, while these are the parts the workflow can act on. `days` applies only
-- to the terms counted from the client receiving the material.
...
CREATE INDEX IF NOT EXISTS order_payment_terms_order_idx
    ON order_payment_terms (order_id, seq);
DROP TRIGGER IF EXISTS order_payment_terms_set_updated_at ON order_payment_terms;
